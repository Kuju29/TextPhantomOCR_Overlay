...
def process_payload(payload: dict, *, admission_identity: str | None = None, admission_unlimited: bool = False) -> dict[str, Any]:
    ...
    reservation = ready_batch.reserve(ai_cfg, lang)
    try:

        # Download errors still precede stage counters inside process_image;
        # credential validation above intentionally precedes image IO.
        try:
        ...
        cache_key = ""
        cache_used = False
        # A saved image result cannot advance a conversation or reflect its current
        # history. HTTP idempotency still owns same-operation replay.
        conversational = bool(ai_cfg and ai_cfg.translation_mode == "conversation")
        if mode in ("lens_images", "lens_text") and img_hash and not conversational:
            ...
            cache_key = cache_mod.build_cache_key(
                img_hash, lang, mode, cache_source, ai_cfg, layout=layout, owner_scope=stage_identity
            )
        ...
        ready_batch.finish(reservation)
