...
process. Native numeric and image libraries may use that number to size thread
pools, so a 2-vCPU Space can accidentally create dozens of runnable threads
and become *slower* as concurrency increases.
...
def cpu_runtime_info() -> dict[str, Any]:
    ...
    # Floor a fractional CPU quota instead of rounding/ceiling it: native
    # threads can be long-running CPU workers and oversubscribing 1.5 quota as two
