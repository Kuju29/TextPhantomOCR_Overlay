...
// --- the service worker reads it onto every payload -------------------------
{
  ...
  assert.ok(
    menu.includes("engineMode: effectiveEngineMode(profileSnapshot.settings.engineMode)"),
    "context-menu.js must gate the seeded stored preference before preflight and payload creation",
  );
  assert.ok(
