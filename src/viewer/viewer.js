...
function renderEmpty() {
  ...
  empty.textContent = "No local images loaded.";
  els.reader.replaceChildren(empty);
  syncUi();
}

function makeSelectionCheckbox(page) {
  const checkbox = document.createElement("input");
  checkbox.type = "checkbox";
  checkbox.dataset.role = "select-page";
  checkbox.dataset.pageId = page.id;
  checkbox.checked = selectedPageIds.has(page.id);
  return checkbox;
}

function makePageMeta(page, index) {
  ...
  meta.textContent = page.relativePath || `Page ${index + 1}`;
  details.append(name, meta);
  return details;
}

function makeStateBadge(page) {
...
function makeActionButton(label, role, pageId) {
...
function makePageListItem(page, index) {
  const item = document.createElement("label");
  item.className = `page-list-item ${selectedPageIds.has(page.id) ? "active" : ""}`;
  ...
  item.append(
    ...
    makePageMeta(page, index),
    makeStateBadge(page),
  );
  return item;
}

function makeReaderPage(page, index) {
  const article = document.createElement("article");
  article.className = `page-strip ${selectedPageIds.has(page.id) ? "active" : ""}`;
  article.dataset.pageId = page.id;

  const head = document.createElement("div");
  head.className = "page-strip-head";
  const title = document.createElement("div");
  title.className = "page-strip-title";

  const titleDetails = makePageMeta(page, index);
  ...
  actions.append(
    makeActionButton("Image", "download-image", page.id),
    makeActionButton("HTML", "download-html", page.id),
  );
  ...
  image.alt = page.name;
  image.loading = "eager";
  image.dataset.tpOriginal = page.originalKey;
  frame.appendChild(image);

  article.append(head, frame);
  return article;
}
...
function renderSession() {
  ...
  els.pageList.replaceChildren(
    ...pages.map((page, index) => makePageListItem(page, index)),
  );
  els.reader.replaceChildren(
    ...pages.map((page, index) => makeReaderPage(page, index)),
  );
